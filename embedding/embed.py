...
for file in os.listdir("text/" + DOMAIN + "/"):
    ...
    with open("text/" + DOMAIN + "/" + file, "r", encoding="UTF-8") as f:
        text = f.read()
        # Replace the last 4 characters to get rid of .txt
        # Replace _ with / to generate the URLs we scraped
        filename = file[:-4].replace("_", "/")
        # There are a lot of contributor.txt files that got included in the
        # scrape, this weeds them out. There are also a lot of auth required
        # urls that have been scraped to weed out as well
        if filename.endswith(".txt") or "users/fxa/login" in filename:
            continue

        # Replace underscores with / to get the actual links
        # so we can cite contributions
